...
pkgbuild --root "$STAGE" --identifier com.onyx.endpoint-agent --version "$VERSION" --install-location / "$OUT/OnyxAgent-$VERSION-macos-universal.pkg"
shasum -a 256 "$OUT/OnyxAgent-$VERSION-macos-universal.pkg" > "$OUT/OnyxAgent-$VERSION-macos-universal.sha256.txt"
