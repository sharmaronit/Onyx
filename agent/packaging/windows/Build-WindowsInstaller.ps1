...
Push-Location $root
try { python -m PyInstaller --clean --noconfirm --onefile --name OnyxAgent --paths $root onyx_agent\__main__.py } finally { Pop-Location }
$wix = Get-Command wix -ErrorAction SilentlyContinue
if (-not $wix) { throw "WiX v4 is required. Install WiX, then rerun this script." }
& $wix.Source build (Join-Path $PSScriptRoot "OnyxAgent.wxs") -o (Join-Path $out "OnyxAgent-$Version-windows-x64.msi")
