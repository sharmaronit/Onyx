...
function Overview() {
  ...
  if (mode === "reality") {
    ...
    if (live.isLoading) return <Shell><div className="p-8">Loading verified live posture...</div></Shell>;
    if (live.error) return <Shell><Panel title="Reality service unavailable"><div className="p-5 text-sm text-destructive">{live.error.message}. Restart the Onyx backend on port 8020, then refresh the browser once.</div></Panel></Shell>;
    const ready = data?.readiness ?? {};
    return <Shell><div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Reality mode · verified customer evidence</p><h2 className="mt-1 text-2xl font-semibold">Security operations overview</h2><p className="mt-1 text-sm text-muted-foreground">Live posture from enrolled endpoints and received telemetry.</p></div><Link to="/telemetry" className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Open incident queue</Link></div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Metric label="Enrolled assets" value={String(data?.enrolled_assets ?? 0)} sub={`${data?.active_assets ?? 0} reporting now`} /><Metric label="Affected assets" value={String(data?.affected_assets ?? 0)} sub={`${data?.open_incidents ?? 0} open incidents`} tone={(data?.affected_assets ?? 0) ? "destructive" : "success"}/><Metric label="Telemetry coverage" value={`${data?.coverage_percent ?? 0}%`} sub={`${data?.telemetry_events ?? 0} received events`} tone="primary"/><Metric label="Observed relationships" value={String(data?.relationships ?? 0)} sub="verified network evidence" /></div>
      <Panel title="Reality-mode readiness"><div className="grid gap-3 p-5 md:grid-cols-4">{Object.entries(ready).map(([key, value]) => <div key={key} className={`rounded-md p-3 text-sm ${value ? "bg-success/10 text-success" : "bg-pearl text-muted-foreground"}`}>{value ? "Ready" : "Required"} · {key.replaceAll("_", " ")}</div>)}</div>{!data?.analytics_ready && <p className="border-t border-hairline p-5 text-sm text-muted-foreground">Risk, exposure, simulation, and patch analytics remain unavailable until live relationship and vulnerability coverage are complete.</p>}</Panel>
    </div></Shell>;
  }

  if (status.isLoading || topology.isLoading) {
    return <Shell><div className="p-8">Loading dashboard...</div></Shell>;
  }

  const kpis = {
    riskScore: status.data?.last_simulation ? Math.round(status.data.last_simulation.success_rate * 100) : 72,
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Portfolio-level risk, exposure, and remediation signals.</p>
          </div>
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(260px,0.8fr)]">
          <Panel title="Latest simulated blast radius">
            <div className="chart-frame ml-0"><MiniMap nodes={nodes} edges={edges} compromised={["web_server_1", "vpn_gateway", "customer_db"]} height={300} /></div>
          </Panel>
          <Panel title="Attack path trend">
            <div className="p-5">
              <div className="flex items-end justify-between"><div><p className="eyebrow">10-week view</p><p className="mt-1 text-2xl font-semibold tabular">{kpis.riskScore}%</p></div><span className="rounded-full bg-primary/10 px-2 py-1 text-[11px] text-primary">current</span></div>
              <div className="mt-5 rounded-md bg-pearl p-3"><Sparkline data={riskTrend} /></div>
              <div className="mt-3 flex justify-between text-[11px] text-muted-foreground"><span>W-10</span><span>Now</span></div>
              <p className="mt-4 text-xs text-muted-foreground">Attack success trend from the latest world-model evaluations.</p>
            </div>
          </Panel>
        </div>
            ...
            <tbody className="divide-y divide-hairline">
              {patches.slice(0, 3).map((p: any, i: number) => (
                <tr key={p.patch_id || p.cve_id || i}>
                  ...
                  <td className="px-5 py-3 text-right tabular">{p.risk_reduction_pp ? (p.risk_reduction_pp * 100).toFixed(1) : p.riskReduction}%</td>
