...
function Simulation() {
  ...
  const nodes = useMemo<MapNode[]>(() => topology.data?.nodes ?? [], [topology.data?.nodes]);
  const edges = useMemo<MapEdge[]>(() => topology.data?.edges ?? [], [topology.data?.edges]);
  ...
  const pathNodeIds = useMemo(
    () => Array.from(new Set(episode.map((item) => item.node_id))),
    [episode],
  );
  const mapNodes = useMemo(() => {
    ...
    return pathNodeIds.map((nodeId, index) => {
      ...
      return {
        ...(node ?? { node_id: nodeId }),
        x: pathNodeIds.length === 1 ? 50 : 10 + (80 * index) / (pathNodeIds.length - 1),
        y: 50,
      };
    });
  ...
  const mapEdges = useMemo<MapEdge[]>(
    () =>
      mapMode === "topology"
        ? edges
        : pathNodeIds.slice(1).map((target, index) => ({
            source: pathNodeIds[index]!,
            target,
            connection_type: "replay",
          })),
    [mapMode, edges, pathNodeIds],
  );
  const validationError =
    episodes < 100 || episodes > 10000
      ? "Episodes must be between 100 and 10,000."
      : seed < 0
  ...
  const saveConfiguration = () => {
    window.localStorage.setItem(
      "onyx-simulation-config",
      JSON.stringify({ topologyKey, episodes, seed }),
    );
    setConfigSaved(true);
    ...
    <Shell>
      <div className="space-y-6">
        <div className="rounded-lg border border-warning/30 bg-warning/5 p-4 text-sm">
          <p className="font-semibold text-warning">Research / simulated environment</p>
          <p className="mt-1 text-muted-foreground">
            Results come from an offline world model. They are not observations of a live breach or
            a measured breach probability.
          </p>
        </div>
        <div className="flex items-center justify-between gap-3 border-b border-hairline pb-3">
          <div className="flex gap-2">
            <button
              onClick={() => setView("run")}
              className={`border-b-2 px-4 py-2 text-sm font-semibold transition-colors hover:text-foreground ${view === "run" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-primary"}`}
            >
              Run simulation
            </button>
            <button
              onClick={() => setView("saved")}
              className={`border-b-2 px-4 py-2 text-sm font-semibold transition-colors hover:text-foreground ${view === "saved" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-primary"}`}
            >
              Saved
            </button>
          </div>
          <button
            ...
            className="rounded-md border border-hairline bg-card px-3 py-2 text-xs font-semibold text-foreground transition-colors hover:bg-pearl"
          >
        ...
        <Panel
          ...
          action={<span className="text-[11px] text-muted-foreground">World-model only</span>}
        >
          <div className="grid items-start gap-4 p-5 md:grid-cols-[1.1fr_1.1fr_1fr_1fr_auto]">
            ...
            <div className="block text-[12px] text-muted-foreground">
              ...
              <div
                title="No packets or endpoint actions are sent."
                className="mt-1 flex min-h-10 items-center rounded-md border border-hairline bg-pearl px-3 py-2 text-[13px] font-medium text-foreground"
              >
                World model simulation
              </div>
              <span className="mt-1 block text-[10px]">
                No packets or endpoint actions are sent.
              </span>
            </div>
            <label className="block text-[12px] text-muted-foreground">
              Evaluation episodes
            ...
            <div className="flex h-full flex-col items-stretch justify-end gap-2 pt-5">
              <button
                disabled={
                  !allowedToRun || simMutation.isPending || !!loadError || !!validationError
                }
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-hairline px-5 py-3 text-[11px] text-muted-foreground">
            <span>
              Estimated runtime: <strong className="text-foreground">~{estimatedSeconds}s</strong>
            </span>
            {validationError && <span className="text-destructive">{validationError}</span>}
            {simMutation.isPending && (
              <span className="font-medium text-primary">Simulation in progress…</span>
            )}
          </div>
          {!allowedToRun && (
        ...
        {view === "saved" && (
          <Panel title="Saved world-model runs">
            <div className="divide-y divide-hairline">
              {savedRuns.isLoading && (
                <p className="p-5 text-sm text-muted-foreground">Loading saved runs…</p>
              )}
              {savedRuns.error && (
                <p className="p-5 text-sm text-destructive">
                  Could not load saved runs: {savedRuns.error.message}
                </p>
              )}
              {!savedRuns.isLoading && !savedRuns.error && savedRuns.data?.runs.length === 0 && (
                <p className="p-5 text-sm text-muted-foreground">
                  No saved runs yet. Run the world model once to create an auditable record.
                </p>
              )}
              {savedRuns.data?.runs.map((run) => (
                <div key={run.simulation_id}>
                  <button
                    onClick={() => {
                      setSelectedSavedId(run.simulation_id);
                      setTopologyKey(run.topology);
                      setEpisodes(run.requested_episodes);
                      setSeed(run.seed);
                      setStep(1);
                    }}
                  ...
                  >
                    <span>
                      ...
                      <span className="mt-1 block text-sm">
                        {run.topology} · seed {run.seed} · {run.completed_episodes.toLocaleString()}{" "}
                        completed
                      </span>
                    </span>
                    <span className="text-right text-xs text-muted-foreground">
                      <span className="block">
                        {percent(run.success_rate)} success · {run.unique_paths} paths
                      </span>
                      <span>{new Date(run.created_at).toLocaleString()}</span>
                    </span>
                  </button>
                  {selectedSavedId === run.simulation_id && savedRun.data && (
                    <div className="border-t border-hairline bg-pearl/45 px-5 py-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="eyebrow">Stored ranked paths</p>
                          <p className="mt-1 text-[11px] text-muted-foreground">
                            {run.topology} · {percent(run.success_rate)} success ·{" "}
                            {run.unique_paths} paths
                          </p>
                        </div>
                        <button
                          type="button"
                      ...
                      <div className="mt-3 max-h-72 space-y-1.5 overflow-y-auto pr-1 text-[12px]">
                        {savedRun.data.paths.map((path) => (
                          <div
                            key={path.rank}
                            className="rounded-md border border-hairline bg-card px-3 py-2"
                          >
                            <span className="mr-2 font-semibold text-primary">#{path.rank}</span>
                            {Array.isArray(path.path) ? path.path.join(" → ") : path.path}
                            <span className="float-right text-[11px] text-muted-foreground">
                              {path.count} runs · {percent(path.frequency)}
                            </span>
                          </div>
                        ))}
                      </div>
            ...
            {showSavedRunModal && savedRun.data && (
              <div
                className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
                role="presentation"
                onMouseDown={(event) => {
                  if (event.target === event.currentTarget) setSelectedSavedId(null);
                }}
              >
                <button
                  type="button"
                  onClick={() => setSelectedSavedId(null)}
                  className="fixed right-6 top-6 z-10 text-2xl text-white hover:text-white/70"
                  aria-label="Close saved run"
                >
                  ×
                </button>
                <div className="max-h-[65vh] w-full max-w-xl overflow-y-auto rounded-lg border border-hairline bg-card p-4 shadow-2xl">
                  <p className="eyebrow">Stored ranked paths</p>
                  {selectedRun && (
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      {selectedRun?.topology} · {percent(selectedRun?.success_rate)} success ·{" "}
                      {selectedRun?.unique_paths} paths
                    </p>
                  )}
                  <div className="mt-3 space-y-1.5 text-[12px]">
                    {savedRun.data?.paths.map((path) => (
                      <div key={path.rank} className="rounded bg-pearl p-3">
                        <span className="mr-2 text-primary">#{path.rank}</span>
                        {Array.isArray(path.path) ? path.path.join(" → ") : path.path}
        ...
        <Panel title="Simulation evidence">
          {!replay?.evidence ? (
            <p className="p-5 text-sm text-muted-foreground">
              Run the world model to record its topology snapshot, seed, and completed episode
              count.
            </p>
          ) : (
            <div className="grid gap-px bg-hairline sm:grid-cols-2 lg:grid-cols-4">
              <div className="bg-card p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {replay.evidence.execution.requested_episodes.toLocaleString()} requested ·{" "}
                  {replay.evidence.execution.successful_runs.toLocaleString()} successful
                </p>
              </div>
              <div className="bg-card p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {replay.evidence.topology.nodes} nodes · {replay.evidence.topology.edges} edges ·
                  hash {replay.evidence.topology.snapshot_sha256}
                </p>
              </div>
              <div className="bg-card p-4">
                <p className="eyebrow">Model & seed</p>
                <p className="mt-2 text-sm font-semibold">
                  {replay.evidence.model.engine.replaceAll("_", " ")}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Seed {replay.evidence.model.seed} ·{" "}
                  {replay.evidence.model.checkpoint_present
                    ? "checkpoint loaded"
                    : "rule-based fallback"}
                </p>
              </div>
        ...
        <Panel
          ...
          action={
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1 border-b border-hairline text-[11px]">
                <button
                  type="button"
                  onClick={() => setMapMode("path")}
                  className={`border-b-2 px-2 py-1.5 font-semibold transition-colors ${mapMode === "path" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-primary hover:text-foreground"}`}
                >
                  Path view
                </button>
                <button
                  ...
                  className={`border-b-2 px-2 py-1.5 font-semibold transition-colors ${mapMode === "topology" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-primary hover:text-foreground"}`}
                >
                  Topology view
                </button>
              </div>
              <span className="text-[12px] text-muted-foreground">
                {replay?.topology ?? topologyKey}
              </span>
            </div>
          }
        >
          {mapMode === "path" && (
            <p className="px-5 pt-4 text-[11px] text-muted-foreground">
              <span className="font-semibold text-destructive">Red</span> shows the simulated route.
              Numbered nodes show the attack order; the ring marks the current replay step.
            </p>
          )}
      ...
      {configModalOpen && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setConfigModalOpen(false);
          }}
        >
          <div
            role="dialog"
            ...
            className="w-full max-w-sm animate-in fade-in slide-in-from-top-2 rounded-lg border border-hairline bg-card p-5 shadow-2xl duration-200"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 id="save-config-title" className="text-base font-semibold">
                  Save world-model configuration
                </h2>
                <p className="mt-1 text-xs text-muted-foreground">
                  Review the parameters before saving.
                </p>
              </div>
              <button
                type="button"
            ...
            <dl className="mt-4 divide-y divide-hairline rounded-md border border-hairline">
              <div className="flex justify-between px-3 py-2 text-xs">
                <dt className="text-muted-foreground">Topology</dt>
                <dd className="font-medium">{topologyKey}</dd>
              </div>
              <div className="flex justify-between px-3 py-2 text-xs">
                <dt className="text-muted-foreground">Environment</dt>
                <dd className="font-medium">World model</dd>
              </div>
              <div className="flex justify-between px-3 py-2 text-xs">
                <dt className="text-muted-foreground">Episodes</dt>
                <dd className="font-medium">{episodes.toLocaleString()}</dd>
              </div>
              <div className="flex justify-between px-3 py-2 text-xs">
                <dt className="text-muted-foreground">Seed</dt>
                <dd className="font-medium">{seed}</dd>
              </div>
              <div className="flex justify-between px-3 py-2 text-xs">
                <dt className="text-muted-foreground">Estimated runtime</dt>
                <dd className="font-medium">~{estimatedSeconds}s</dd>
              </div>
            </dl>
            <div className="mt-5 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setConfigModalOpen(false)}
                className="rounded-md border border-hairline px-3 py-2 text-xs font-medium text-secondary-foreground hover:bg-pearl"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-md bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground disabled:bg-chip disabled:text-muted-foreground"
              >
                Save configuration
              </button>
            </div>
